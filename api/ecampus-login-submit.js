...
export default async function handler(req, res) {
  ...
  try {
          ...
          // The full SAML/Duo redirect chain can take longer than a couple of
          // seconds, so poll for up to ~20s rather than checking once.
          let mainPage = null;
            let isLoggedIn = false;
            for (let i = 0; i < 10; i++) {
                        allPages = await browser.pages();
                        mainPage = allPages.find(p => p.url().includes('ecampusd2l.blinn.edu'));
                        if (mainPage) {
                                      await mainPage.bringToFront();
                                      isLoggedIn = await mainPage.$('.d2l-navigation') !== null;
                                      if (isLoggedIn) break;
                        }
                        await sleep(2000);
            }
          ...
          if (!isLoggedIn) {
                      return res.status(400).json({
                                    ...
                                    currentUrl: mainPage ? mainPage.url() : allPages.map(p => p.url()),
                                    message: 'The code may have been wrong or expired, or the redirect is still catching up. Try /api/ecampus-promote in a moment, or /api/ecampus-login-init again.',
                      });
          }
